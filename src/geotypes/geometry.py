...
class LL(GeoType):
    ...
    def distance(self, other: 'LL') -> float:
        """
        Calculates the distance between two LL points
        """
        return geopy.distance.distance((self.lat, self.lon), (other.lat, other.lon)).meters
...
class XY:
    ...
    def translate_range_bearing(self, range, bearing, degrees: bool = False, compass_heading: bool = True) -> 'XY':
        """
        Translate the XY point by a range and bearing
        ...
        if compass_heading:
            a = -(a - np.pi / 2)
        x = self.x + range * np.cos(a)
        y = self.y + range * np.sin(a)
        return XY(x, y)
...
class GeoPoint(GeoType):
    ...
    def __copy__(self):
        """
        Returns a copy of the GeoPoint object.

        Returns:
            GeoPoint: A copy of the GeoPoint object.
        """
        return GeoPoint(self.x, self.y, self.crs)

    def copy(self):
        """
        Copy the GeoPoint object
    ...
    def distance(self, other: GeoType):
        ...
        if crs.is_geographic:
            ...
            return self.as_geoseries().distance(other.as_geoseries())[0]
...
class LocalPath:
    ...
    def __getitem__(self, item: int) -> Union[XY,'LocalPath']:
        """
        ...
        if isinstance(item, slice):
            ...
            return LocalPath([XY(ret[i][0], ret[i][1]) for i in range(len(ret))])
        elif isinstance(item, int):
            if item < 0:
                item = len(self.ls.coords) + item
            if item >= len(self.ls.coords):
    ...
    def to_xys(self) -> List[XY]:
        """
        """
        return [XY(p[0], p[1]) for p in self]
...
class GeoPath(GeoType):
    ...
    def reverse(self) -> 'GeoPath':
        """
        Reverses the path
        """
        new_points = []
        for n in range(len(self)-1, -1, -1):
            new_points.append(self[n])
        return GeoPath(new_points)
...
def load_geometries(path: str) -> List['GeoType']:
    if not os.path.isfile(path):
        raise ValueError("Invalid path")
    geometry_list = []
    gdf = gpd.read_file(path).explode(index_parts=True)
...
def load_named_geometries(path: str, name_attribute: str) -> Dict[str, 'GeoType']:
    """
    Loads a set of geometries from a file, with a name attribute.
    """
    gdf = gpd.read_file(path).explode(index_parts=True)
    geometry_lookup = {}
    for index,row in gdf.iterrows():
        if name_attribute is None or name_attribute == 'index':
            name = index[0]
        else:
            if not name_attribute in row:
                raise ValueError(f"Name attribute {name_attribute} not found in row {index}")
            name = row[name_attribute]

        if row.geometry.geom_type == "Point":
            geometry_lookup[name] = GeoPoint(row.geometry.x, row.geometry.y, crs=gdf.crs)
        elif row.geometry.geom_type == "MultiPoint":
            geometry_lookup[name] = GeoPoints(row.geometry,crs=gdf.crs)
        elif row.geometry.geom_type == "LineString":
            geometry_lookup[name] = GeoPath(row.geometry, crs=gdf.crs)
        elif row.geometry.geom_type == "Polygon":
            geometry_lookup[name] = GeoArea(row.geometry, crs=gdf.crs)
...
def get_utm_epsg(ll: LL) -> str:
    """
    Gets the UTM EPSG code for a given LL.
    Args:
        ll: (LL) The LL to get the UTM EPSG code for.
    Returns:
        str: The UTM EPSG code.
    """
    # Determine UTM zone
    utm_zone = int((ll.lon + 180) // 6) + 1
    # Determine if it's in the northern or southern hemisphere
    hemisphere = 'north' if ll.lat >= 0 else 'south'
